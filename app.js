...
function setMode(mode) {
  ...
  els.undoBtn.disabled = !((mode === "trace" && state.currentTrace.length) || (mode === "dimension" && state.pendingDimensionPoints.length));
  render();
}
...
async function handleFile(file) {
  ...
  state.activeDimensionIndex = -1;
  state.scaleDimensionId = null;
  state.filename = file.name.replace(/\.[^.]+$/, "") || "drawing";
  setLoadedUi();
  updateReview();
...
function undoPoint() {
  ...
  els.undoBtn.disabled = !((state.mode === "trace" && state.currentTrace.length) || (state.mode === "dimension" && state.pendingDimensionPoints.length));
  render();
}
...
function updateReview({ preserveInputs = false } = {}) {
  ...
  for (const el of [
    els.dimensionLabel, els.dimensionValue, els.confirmDimensionBtn, els.useScaleBtn,
    els.ignoreDimensionBtn, els.prevDimensionBtn, els.nextDimensionBtn,
  ...
  els.dimensionHeading.textContent = d.status === "ignored" ? "Ignored dimension" : d.status === "confirmed" ? "Confirmed dimension" : "Needs confirmation";
  if (!preserveInputs) {
...
function updateExportState() {
  const scale = mmPerPixel();
  const canExport = Boolean(scale && state.cutShapes.length);
  ...
  if (canExport) {
    els.exportMessage.textContent = "Ready to export. Check the traced geometry before using the DXF for production.";
  } else if (!scale) {
    els.exportMessage.textContent = "Choose a dimension with a known value and press Use as scale.";
  } else {
...
function drawDimension(d, index, active) {
  ...
  const label = d.valueMm ? `${index + 1}: ${d.valueMm} mm` : `${index + 1}: ?`;
...
function render() {
  ...
  if (state.loaded) ctx.drawImage(sourceCanvas, 0, 0);
  state.cutShapes.forEach((shape) => drawShape(shape.points, true));
  if (state.currentTrace.length) drawShape(state.currentTrace, false, "#b54708");
  state.dimensions.forEach((d, index) => drawDimension(d, index, index === state.activeDimensionIndex));
  for (const p of state.pendingDimensionPoints) drawHandle(p, "#2563eb", 6);
}
...
els.canvas.addEventListener("pointerdown", (event) => {
  ...
  else if (state.mode === "trace") addTracePoint(point);
});
